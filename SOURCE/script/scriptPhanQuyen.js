...
var strRequestData =
  '{"lstPermissionFunction":[{"functionId":"{0}","permissionAppId":"{1}","appCode":"{2}","isAccess":"1","createBy":"{3}","viewActionId":{4},"createActionId":0,"updateActionId":{5},"deleteActionId":0,"importActionId":0,"exportActionId":0}]}';
var strGetPerFunctions =
  '{"AppId":{0},"FunctionId":0,"PermissionAppId":"{1}","PageNumber":1,"PageSize":500}';
var strFunction =
  '[{"isEnable":"{0}","code":"{1}","displayName":"{2}","path":"{3}","orders":"{4}","parentCode":"{5}","appCode":"{6}","createBy":"{7}"}]';
var strRoleUser =
  '{"lstRoleUsers":[{"id":0,"roleCode":"{0}","userName":"{1}","isDelete":0,"createBy":"{2}"}]}';

// Thêm quyền user
async function AddRoleUser(roleCode, userName) {
  input = strRoleUser.f(roleCode, userName, createBy);
  var result = await callAjax(
    "POST",
    "/api/user/v1/User/AddRoleUsers_v2",
    input,
    strBearer
  ).then((res) => {
    return setDefaultIsNull(res);
  });
...
async function GetRoleUser(userName) {
  ...
  var result = await callAjax(
    "GET",
    "/api/user/v1/User/GetUserRoleByEmail",
    input,
    strBearer
  ).then((res) => {
    return setDefaultIsNull(res);
...
async function GetInfoByUser(userName) {
  ...
  if (infoUser) {
    ...
    console.log(
      infoUser.positionName,
      infoUser.divisionName,
      infoUser.departmentName
    );
  }
  ...
  var PermissionApps = await callAjax(
    "POST",
    "/api/user/v1/permissionapp/GetPermissionApps",
...
async function DelRoleUser(idRole, userName) {
  ...
  var result = await callAjax(
    "DELETE",
    "/api/user/v1/roleuser/Delete_v2",
    input,
    strBearer
  ).then((res) => {
    return setDefaultIsNull(res);
...
async function UpdateMenu(isShow = 0) {
  ...
  for (var v of listMenu) {
    ...
    if (result.code != 200) {
      ...
      var resultPut = await callAjax(
        "PUT",
        "/api/user/v1/function/Put",
        input,
        strBearer
      ).then((res) => {
        return setDefaultIsNull(res);
...
async function delFunction(functionId) {
  var result = await callAjax(
    "DELETE",
    "/api/user/v1/function/Delete?Id={0}".f(functionId),
    null,
    strBearer
  ).then((res) => {
    return setDefaultIsNull(res);
...
async function getPermissionFunctions(_permissionAppId = 0) {
  //permissionAppId khai báo ở trên.
  input = strGetPerFunctions.f(appId, _permissionAppId ?? permissionAppId);
  var result = await callAjax(
    "POST",
    "/api/user/v1/permissionfunction/GetPermissionFunctions",
    input,
    strBearer
  ).then((res) => {
    return setDefaultIsNull(res);
...
async function runPermission(action = 0) {
  //action = 0 : View, 1: View + Update
  ...
  for (var p of listPer) {
    if (perOld != p.functionCode) {
      ...
      var result = await callAjax(
        "POST",
        "/api/user/v1/permissionfunction/AddPermissionFunctions",
        input,
        strBearer
      ).then((res) => {
        return setDefaultIsNull(res);
...
async function delAllPermissionByRole(type = 0, strfilter = "") {
  ...
  listFunctionDel = listFunction.filter(
    (i) =>
      i.functionCode.includes(strfilter) &&
      (type == 0 ? i.functionCode == strfilter : 1 == 1)
  );
...
async function delPermission(perId) {
  var result = await callAjax(
    "DELETE",
    "/api/user/v1/permissionfunction/Delete?Id={0}".f(perId),
    null,
    strBearer
  ).then((res) => {
    return setDefaultIsNull(res);
...
function list() {
  ...
  console.log(
    "%setstrPermissionProd()",
    "color: #bada55",
    "\n\t Cập nhật string Permission, permissionAppId Product"
  );

  console.log(
    "%c Cập nhật các biến trước khi thao tác tiếp: strPermission  strMenu  strBearer  strFunction  permissionAppId  createBy  appCode appId",
...
$(document).ready(function () {
  ...
  if (strBearer == "") {
    console.log(
      "%c strBearer chưa được cập nhật, vui lòng bổ sung.",
      "color: RED"
    );
  } else {
